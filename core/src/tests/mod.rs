...
fn test_aais_019_state_commission_visibility() {
    ...
    const FULL_REFUND_TEXT: &str = "UBS Europe SE will refund to you the full amount less expenses";
    ...
    fn has_company_field(state: &crate::FormState) -> bool {
        state
            .flattened
            .iter_nodes()
            .any(|n| matches!(&n.kind, FlattenedNodeKind::Field { name, .. } if name == "Company"))
    }
    ...
    for state in states.iter() {
        ...
        let is_firma_or_gbr = client_types.iter().any(|ct| ct == "Firma" || ct == "GbR");
...
fn diag_acroform_second_pdf() {
    ...
    use crate::pdf_parser::acroform::{extract_acroform_fields, AcroFieldType};
    ...
    let mut page_id_to_index: std::collections::HashMap<lopdf::ObjectId, usize> = std::collections::HashMap::new();
    ...
    for f in &fields {
        if f.field_type == AcroFieldType::Button {
            println!(
                ...
                f.name, f.value, f.is_checked, f.rect, f.page_index.unwrap_or(999)
            );
        }
    }
