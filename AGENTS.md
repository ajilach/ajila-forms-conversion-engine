...
- When running tests, use `partest run --release` such that they are faster. This is like `cargo test`, but distributes across multiple machines, so cargo test does not need to be run as well.
