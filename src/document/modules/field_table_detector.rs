...
//!
//! Detects tables of fields with bold column headers and converts them into
...
impl FieldTableDetector {
    ...
    fn detect_field_tables(&self, doc: &mut Document) {
        ...
        let fields: Vec<usize> = all_roots
            .iter()
            .filter(|&&idx| {
                doc.get_group(idx)
                    .map(|g| matches!(g.kind, GroupKind::Field | GroupKind::DateField { .. }))
                    .unwrap_or(false)
            })
            .copied()
            .collect();
        ...
        let bold_text_blocks: Vec<usize> = all_roots
            .iter()
            .filter(|&&idx| {
                doc.get_group(idx)
                    .map(|g| matches!(g.kind, GroupKind::TextBlock))
                    .unwrap_or(false)
                    && self.is_all_bold(doc, idx)
            })
            .copied()
            .collect();
