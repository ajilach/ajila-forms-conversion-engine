...
use blueprint::{
    ...
    GraphSelection, GraphState, HtmlConfig, HtmlCustomStyles, MergeInput, RecursiveMerger,
    Selection, StructuredNode, build_field_label_map, document, flattened, structured, xfa,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for doc_path in &args.documents {
        ...
        if need_structured && !structured_outputs.is_empty() {
            ...
            if args.aem && args.documents.len() <= 1 {
                if bp.is_xfa() {
                    let aem_config =
                        load_aem_config(args.profile.as_deref(), &merged_envelope.context)?;
                    let aem_output =
    ...
    if args.documents.len() > 1 && !all_envelopes.is_empty() {
        ...
        if args.aem {
            let aem_config = load_aem_config(args.profile.as_deref(), &merged_envelope.context)?;
